#!/bin/bash
# Test script for IP-based Engine ID: 0x80000000c001c0a8010a

TEST_ID="IP-ENGINE-$(date +%s)"

echo "=== Testing SNMPv3 with IP-Based Engine ID ==="
echo "IP Address: 192.168.1.10"
echo "Engine ID: 0x80000000c001c0a8010a"
echo "Test ID: $TEST_ID"
echo

echo "Sending SNMPv3 trap with IP-based Engine ID..."
snmptrap -v 3 -e 0x80000000c001c0a8010a -u NCEadmin -a SHA -A P@ssw0rdauth -x AES -X P@ssw0rddata \
...
echo "<snmp_trap><timestamp>$(date)</timestamp><engineID>0x80000000c001c0a8010a</engineID><id>$TEST_ID-DIRECT</id></snmp_trap>" | nc -u 165.202.6.129 1237
