#!/bin/bash
# Script to set up SNMPv3 with MD5 authentication

# SNMPv3 Credentials
SNMPV3_USER="NCEadmin"
AUTH_PROTOCOL="MD5"  # Changed from SHA to MD5
AUTH_PASS="P@ssw0rdauth"
PRIV_PROTOCOL="AES"
PRIV_PASS="P@ssw0rddata"
CONTEXT_NAME=""  # Optional, leave blank if not needed

echo "=== Setting up SNMPv3 with MD5 Authentication ==="
...
cat > snmpv3-md5.conf << EOF
# SNMPv3 configuration with MD5 authentication
...
echo "createUser $SNMPV3_USER $AUTH_PROTOCOL $AUTH_PASS $PRIV_PROTOCOL $PRIV_PASS" > persistent-snmpv3.conf

# Apply the configurations to the container
echo "Applying configuration to container..."
cat snmpv3-md5.conf | docker exec -i fluentd-snmp-trap tee /etc/snmp/snmptrapd.conf > /dev/null
cat persistent-snmpv3.conf | docker exec -i fluentd-snmp-trap tee /var/lib/net-snmp/snmptrapd.conf > /dev/null
echo "Configuration applied."

# Create a test script for sending traps without specifying Engine ID
echo "Creating test script..."
cat > test-snmpv3-md5.sh << EOF
#!/bin/bash
# Test script for SNMPv3 with MD5 authentication
...
TEST_ID="SNMPV3-MD5-\$(date +%s)"

echo "=== Testing SNMPv3 with MD5 Authentication ==="
echo "Username: \$SNMPV3_USER"
echo "Auth Protocol: \$AUTH_PROTOCOL"
echo "Privacy Protocol: \$PRIV_PROTOCOL"
...
echo "Sending SNMPv3 trap with MD5 authentication..."
...
if docker exec fluentd-snmp-trap cat /var/log/snmptrapd.log | grep -q "\$TEST_ID"; then
  echo "✅ SUCCESS: SNMPv3 trap with MD5 auth was received!"
  echo "Log entry:"
...
echo "<snmp_trap><timestamp>\$(date)</timestamp><version>SNMPv3</version><user>\$SNMPV3_USER</user><auth>MD5</auth><id>\$TEST_ID-DIRECT</id></snmp_trap>" | nc -u 165.202.6.129 1237
echo "✅ Direct UDP message sent to 165.202.6.129:1237"
EOF

chmod +x test-snmpv3-md5.sh
